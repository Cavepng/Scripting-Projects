...
model  = "gemini-3-flash-preview" ' Use gemini-2.0-flash if 3.0 is busy

' --- GET USER INPUT ---
userPrompt = InputBox("Enter your prompt for Gemini:", "Gemini AI Assistant")
...
If Err.Number <> 0 Then
    MsgBox "Error connecting to Gemini: " & Err.Description, 16, "Connection Error"
    WScript.Quit
End If
On Error GoTo 0
...
' --- PARSE JSON (The "All-in-One" Way) ---
' We use a hidden HTML document to use its JSON.parse capability
Dim html, window, parsedText
Set html = CreateObject("htmlfile")
Set window = html.parentWindow

' Add a small JS helper to safely grab the specific text field
window.execScript "function getGeminiText(jsonStr) { " & _
    "  var obj = JSON.parse(jsonStr); " & _
    "  return obj.candidates[0].content.parts[0].text; " & _
    "}", "JScript"

On Error Resume Next
parsedText = window.getGeminiText(responseJSON)

If Err.Number <> 0 Then
    MsgBox "Could not parse AI response. Raw data: " & vbCrLf & responseJSON, 48, "Parsing Error"
Else
    ' Output the clean text
    MsgBox parsedText, 64, "Gemini Response"
End If
...
' --- CONFIGURATION ---
Dim apiKey, model, userPrompt
apiKey = "YOUR_API_KEY_HERE"  ' <--- ENSURE THIS IS CORRECT
model  = "gemini-1.5-flash"   ' Using a very stable model for VBScript
